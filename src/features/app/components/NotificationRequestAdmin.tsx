...
export const NotificationRequestAdmin = ({ display }: { display: string }) => {
    const { data, loading } = useNotificationRequestStore();
    const { error, updateNotification } = useNotificationRequest();

    // DEBUG
    // console.log("Notification ID: ", data?.map((req) => req.id));
    // console.log("Status ID: ", data?.map((req) => req.status_id));
            ...
            <div className="flex flex-col gap-2 overflow-y-auto max-h-[30vh] sm:max-h-[50vh] pr-0.5">
                {data && data.length > 0 ? data.map((item) => (
                    ...
                    >
                        ...
                        {/* Badge */}
                        <span className={`text-xs font-medium px-2 py-1 sm:px-2.5 rounded-lg ${STATUS_CLASS[item.status_id] ?? "bg-gray-100 text-gray-500"}`}>
                            {STATUS_LABEL[item.status_id] ?? "Unknown"}
                        </span>


                        <div className="flex flex-row gap-1">
                            <button type="button" onClick={() => updateNotification(item.id, "2")} className="bg-primary p-2 hover:bg-lime-700 cursor-pointer rounded-xl text-white"><i className="ri-check-line"></i></button>
                            <button type="button" onClick={() => updateNotification(item.id, "3")} className="bg-red-700 p-2 hover:bg-red-800 cursor-pointer rounded-xl text-white"><i className="ri-close-line"></i></button>
                        </div>
                    </div>
                )) : (
